...
func _ready():
	...
	add_autoload_singleton("MatrixCodex", "res://addons/matrix_codex/matrix_codex.gd")

func _exit_tree():
	remove_custom_type("CodexEntry")
	remove_tool_menu_item(TOOL_MENU_LABEL)
	codex_menu.queue_free()
	remove_autoload_singleton("MatrixCodex")

func _open_codex(ud):
	codex_menu.get_node("MainWindow").popup_centered()
